...
The live site is served from `resume-frontend/public/`, not from a Jekyll build. Local Ruby is 2.6, which is too old for current Jekyll, so posts are rendered by `tools/build_post_html.py` instead. That script reuses an already-published page (`2024/12/08/robot-arm-advml/index.html`) as its layout shell, so the generated pages stay consistent with the Jekyll-built ones.

**Workflow:**

1. Write the post in `_posts/YYYY-MM-DD-slug.md` with YAML front matter (`layout`, `title`, `date`, `description`, `excerpt`).
2. Put figures in `assets/<project>/` and copy them to `resume-frontend/public/assets/<project>/`.
3. Render to the public site:

   ```bash
   python tools/build_post_html.py _posts/YYYY-MM-DD-slug.md ../resume-frontend/public
   ```

   Output lands at `resume-frontend/public/YYYY/MM/DD/slug/index.html`.
4. To surface the post on a project page, add an action to that project's entry in `resume-frontend/public/data/project-manifest.json`:
...
- **Stylesheet is mirrored.** `css/main.css` and `resume-frontend/public/css/main.css` must stay identical; edit both. Browsers cache it aggressively, so hard-reload when verifying style changes.
